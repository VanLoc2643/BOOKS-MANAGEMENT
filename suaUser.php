...
<?php
    ...
    if(isset($_GET['id']) && is_numeric($_GET['id'])){
        ...
        $querySelect = "SELECT * FROM `nguoidung` WHERE `id` = $userId";
            ...
            <div class="container">
                <form action="capnhatuser.php" method="POST">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="sodienthoai" name="sodienthoai" value="<?php echo $sodienthoai; ?>" required>
                    </div>
...
?> 
   ...
   document.getElementById("username").addEventListener("input", function() {
    ...
    if (inputValue.length > 16) {
        alert("Tên tài khoản không được vượt quá 16 kí tự.");
...
document.getElementById("sodienthoai").addEventListener("input", function() {
    var inputValue = this.value.trim();
    var numbers = /^[0-9]+$/;
    if (!inputValue.match(numbers) || inputValue.length > 12) {
        alert("Số điện thoại không hợp lệ, vui lòng nhập lại (tối đa 12 chữ số).");
        this.value = ""; // Xóa giá trị nếu không hợp lệ
    }
});
